...
import { Download, FileText, Calculator, Loader2, TrendingUp } from '@/icons/lucide'
...
export function AnalysisResults({ result, isAnalyzing }: AnalysisResultsProps) {
  ...
  const handlePredict = async () => {
    ...
    try {
      const response = await axios.post(`${config.api.baseUrl}${config.api.endpoints.predict}`, {
  ...
  const handleDownloadReport = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${config.api.baseUrl}${config.api.endpoints.generatePdf}`, formData, {
        headers: {
          ...(session && { 'Authorization': `Bearer ${session.user.email}` })
        }
      });
      ...
      if (response.data?.job_id) {
        ...
        setTimeout(() => {
            ...
            link.href = `${config.api.baseUrl}${config.api.endpoints.downloadReport}/${job_id}`;
